...
describe("saveConfig merge保存", () => {
  test("部分更新しても既存キーが保持される", () => {
    ...
    saveConfig({ pollExclude: ["account-c"] }, path);
    const cfg = loadConfig(path);
    assert.equal(cfg.pollMinutes, 5);
    assert.deepEqual(cfg.pollExclude, ["account-c"]);
    ...
    assert.deepEqual(cfg2.pollExclude, ["account-c"]);
  });
...
describe("getPollExclude / setPollExclude", () => {
  test("配列でない・文字列でない値は無視される", () => {
    ...
    writeFileSync(path, JSON.stringify({ pollExclude: "account-c" }));
