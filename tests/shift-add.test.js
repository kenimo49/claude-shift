...
describe("shift add — duplicate token detection", () => {
  test("同一トークンが既に別ラベルで登録済みなら中止", () => {
    ...
    try {
      const isolated = makeIsolatedHome({
        ...
        accounts: { "account-a": "sk-ant-oat01-SAME" },
      });
      try {
        const r = runShift(isolated, ["add", "account-a-copy"]);
        assert.notEqual(r.code, 0);
        assert.match(r.stderr, /既に別アカウント 'account-a'/);
        assert.ok(!existsSync(join(isolated, ".claude-shift", "accounts", "account-a-copy.json")));
      } finally {
  ...
  test("-f で重複警告を無視して強制登録", () => {
    ...
    try {
      const r = runShift(isolated, ["add", "account-a-copy", "-f"]);
      assert.equal(r.code, 0);
      assert.match(r.stdout, /Saved as:/);
      const saved = JSON.parse(
        readFileSync(join(isolated, ".claude-shift", "accounts", "account-a-copy.json"), "utf8")
      );
  ...
  test("同名アカウントへの上書きは警告なしで成功", () => {
    const isolated = makeIsolatedHome({
      ...
      accounts: { "account-a": "sk-ant-oat01-OLD" },
    });
    try {
      const r = runShift(isolated, ["add", "account-a"]);
      assert.equal(r.code, 0);
      const saved = JSON.parse(
        readFileSync(join(isolated, ".claude-shift", "accounts", "account-a.json"), "utf8")
      );
