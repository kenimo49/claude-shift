...
```
Saved setup-token for 'account-c' (login併存)
  expires: 2027/1/1 (365日後)
  identity: alice@example.com (uuid=1a2b3c4d-...)
```
...
```
Accounts:
  * account-c [login+token] (active: login)  token期限: 2027-01-01 (残365日)
    account-b [login+token]  token期限: 2027-01-01 (残365日)
    account-a [login+token]  token期限: 2027-01-01 (残365日)
```
...
| マシン | 実行 | 観測 |
|---|---|---|
| メインPC | /login（従来どおり。Remote Control 等のフル機能） | 自分が login を所有するアカウントのみ |
| サブマシン | setup-token（`use-token` で既定化） | 他マシン所有のアカウントは `observe <name> off` で除外 |
