...
```
⚠️  現在の credentials.json のトークンは既に別アカウント 'account-a' として登録されています。
中止しました。
```
...
```bash
bash shift.sh add account-a-copy -f
```
...
`/login` の画面で別アカウントを選択してしまうと、`credentials.json` はそのアカウントで上書きされます。この状態で `shift add account-a` すると、account-a.json に別アカウントのトークンが保存されて誤ラベル化します。
...
bash shift.sh add-token account-a # 貼り付けて登録 (入力は非表示)。pipe も可
bash shift.sh list             # account-a [login+token] token期限: YYYY-MM-DD (残N日)
```
...
- 使うときは環境変数で: マシンの既定にするなら `bash shift.sh use-token account-a`（`~/.claude-shift/env.sh` に書き出し、`.bashrc` から source）、単発なら `eval "$(bash shift.sh env account-a)"` や `CLAUDE_CODE_OAUTH_TOKEN=$(bash shift.sh token account-a) claude -p "..."`
- **`shift use` は setup-token では動きません** (credentials.json に書くと claude CLI が refresh を試みて壊れるため、token-only アカウントには use を拒否してガイダンスを出します)。`use` を実行すると `env.sh` は自動削除され、login モードに戻ります（token/login は排他）

### 複数マシン運用の原則（実行と観測の分離）

同一アカウントを複数マシンで使う場合の割り当てルール:

1. **実行**: login (/login) は1台だけ。他のマシンは setup-token（`use-token`）で動かす
2. **観測**: usage ポーリング（`shift server` / `shift usage`）も login credentials の refresh を消費するため、**login を所有する1台だけ** が観測する。他のマシンでは `bash shift.sh observe <name> off` で観測から除外する（設定は `~/.claude-shift/config.json` の `pollExclude`、Chrome 拡張の ⚙ 設定からも変更可）

除外したアカウントは `list` と popup に「観測対象外」と表示されます。手順の全体像は [setup-token-guide.md](setup-token-guide.md) §4 を参照。

## `shift use` の挙動

```bash
bash shift.sh use account-a
```

内部で以下を実行します（`cli/accounts.js` に委譲）：

1. sync-back: 現在の `credentials.json` を、現在アクティブなアカウントファイルへ書き戻す（refresh 済みトークンを保全）
2. `account-a.json` の内容を `credentials.json` にコピー
