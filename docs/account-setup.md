...
- 使うときは環境変数で: マシンの既定にするなら `bash shift.sh use-token kumiko`（`~/.claude-shift/env.sh` に書き出し、`.bashrc` から source）、単発なら `eval "$(bash shift.sh env kumiko)"` や `CLAUDE_CODE_OAUTH_TOKEN=$(bash shift.sh token kumiko) claude -p "..."`
