...
個人の Max と仕事の Team など、複数の Claude Code アカウントを使い分けようとすると 2 つの壁に当たります。

1. **切替が 2 ファイル問題を踏む**: Claude Code は認証 token (`~/.claude/.credentials.json`) と表示情報 (`~/.claude.json` の `oauthAccount`) を **別ファイルで管理**しています。素朴に `credentials.json` だけ差し替えると `/status` が古いアカウントを表示し続け、refresh 済み token を捨てて 401 になる事故も起きます。詳細は [docs/knowledge/claude-code-auth-internals.md](docs/knowledge/claude-code-auth-internals.md) と [解説記事 (kenimoto.dev/ja)](https://kenimoto.dev/ja/blog/claude-code-two-file-auth-multi-account/) にまとめてあります。
2. **複数マシンで毎日ログアウトされる**: 同一アカウントを 2 台で `/login` すると、refresh token のローテーション競合で片方が約 1 日周期で認証切れになります ([docs/knowledge/multi-device-token-conflict.md](docs/knowledge/multi-device-token-conflict.md))。

`shift use` は 2 ファイルを一貫して書き換えつつ refresh 済み token の sync back までやり、複数マシンは setup-token 併用 (`use-token` / `observe off`) で競合経路を塞ぎます。各アカウントの 5 時間枠・週次枠の消費状況は CLI / Chrome 拡張から横断で観測できます。
...
```bash
# メインマシン: 通常どおり /login (このマシンが login を所有 = 観測もここ)
